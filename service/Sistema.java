...
public class Sistema {
    ...
    public static boolean VerficadorData(String data, Paciente paciente) throws ParseException {
        ...
        for (int i = 0; i < ListaAutorizacao.size(); i++) {
            if (ListaAutorizacao.get(i).getPaciente() == paciente) {
                ...
                if (trintaDiasDepois.isAfter(dataNova) && dataAutorizacao.isBefore(dataNova)) { // verifica se é 30 dias
                                                                                                // depois
                    return false;
                }

                if (trintaDiasAntes.isBefore(dataNova) && dataAutorizacao.isAfter(dataNova)) { // verifica se é 30 dias
                                                                                               // antes
                    return false;
                }

                if (dataNova.isEqual(trintaDiasAntes) || dataNova.isEqual(trintaDiasDepois)
                        || dataNova.isEqual(dataAutorizacao)) { // verifica se as datas sao iguais
                    return false;
                }
            }
        }
    ...
    public static boolean adicionarAdministrador(Administrador administrador) {
        ...
        for (int i = 0; i < ListaAdministrador.size(); i++) {
            if (ListaAdministrador.get(i).equals(administrador)
                    || ListaAdministrador.get(i).getId() == administrador.getId()) {
                return false;
            }
        }
